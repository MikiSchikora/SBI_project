...
parser.add_argument('-o', '--output-dir', metavar='\b',
   dest="outputdir",
   action="store",
   default="./output_models/",
   help="Output directory")

parser.add_argument('-i', '--input', metavar='\b',
   dest="input",
   action="store",
   default=None,
...
parser.add_argument('-seq', '--sequences', metavar='\b',
   dest="sequences",
   action="store",
   default=None,
...
parser.add_argument('-sto', '--stoichiometry', metavar='\b',
   dest="stoich",
   action="store",
   default=None,
   help="Add a tabular file with sequence ID and stoichiometry")

parser.add_argument('-n_models', '--number-of-models', metavar='\b',
   dest="n_models",
   type=int,
   action="store",
   default=1,
...
parser.add_argument('-n_chains', '--number-of-chains', metavar='\b',
   dest="n_chains",
   type=int,
   action="store",
   default=1000,
...
if options.input:
    ...
    if os.path.isfile(options.input):
        ...
        # when the input is a file (used in testing)
        # check for file type
...
# print warning message if exhaustive option has been specified
...
# handle multifasta file input: a file containing the sequences of the subunits to include. This is mandatory for any chain that is not a random DNA seuqnce.
...
if options.stoich:
    # stoichiometry has to be input together with sequences
    if options.sequences:
